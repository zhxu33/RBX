...
ChatService:RegisterProcessCommandsFunction("default_commands", function(fromSpeaker, message, channel)
	...
	elseif table.find(Commands, args[1]) then
		return true
	...
	for FilterIndex, Word in pairs(Filter1) do
		if string.find(string.lower(message), Word) then
			for FilterIndex2, Word2 in pairs(Filter2) do
				if string.find(string.lower(message), Word2) then
					return true
				end
			end
		end
