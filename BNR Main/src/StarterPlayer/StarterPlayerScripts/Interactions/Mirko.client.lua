...
local Interacted =  workspace:WaitForChild("InteractionNPCS"):WaitForChild("Mirko"):WaitForChild("Interact")
...
AcceptQuest.OnClientEvent:Connect(function(Quest)
	if Quest ==  "Kill High End" then
		NPCType.Text = "Defeat High End"
...
CompleteQuest.OnClientEvent:Connect(function(Quest, questRewards)
	if Quest == "Kill High End" then
		...
		questReward:CreatePane(QuestGui, questRewards, "Kill High End")
	end
end)
...
Interacted.MouseClick:Connect(function()
	if InteractGui.Enabled == false then
		...
		if Quest.Name == "Kill High End" then
		...
		CompleteQuest:FireServer("Kill High End")
		...
		local fullmsg = tostring("Hey you're a hero right? Help us take care of these high ends.")
		for i = 1, fullmsg:len() do
			game:GetService("RunService").RenderStepped:wait()
			Response.Text = Response.Text .. fullmsg:sub(i,i)
		end
		...
		AcceptQuest:FireServer("Kill High End")
		Response.Text = ""
