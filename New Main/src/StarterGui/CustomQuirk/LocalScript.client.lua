...
local PhantomAccess = Quirks.QuirkAccess.Phantom
...
for i=1,#PhantomAccess do
	if Player.Name == PhantomAccess[i] then
		Yes = true
		break
	end
end
...
if Yes then
	...
	local Phantom = game.ReplicatedStorage.CustomQuirk:WaitForChild("Phantom")
	...
	for i=1,#DIOAccess do
		...
		if Available8 then
			CustomFrame.Availables.Custom8.MouseButton1Click:Connect(function()
				if not CustomOn then
					CustomOn = true
					Phantom:FireServer()
					script.Parent:Destroy()
			...
			CustomFrame.Availables.Custom8.Position = Rows[Row]
			break
		end
	end
	
	if not Available8 then
		CustomFrame.Availables.Custom8:Destroy()
	end
end