...
local QuestManager = require(game.ReplicatedStorage.Modules.Quests.QuestManager)
...
game.Players.PlayerAdded:Connect(function(player)
	...
	if PlayerQuestInfo ~= nil then
		...
		AcceptQuest:FireClient(player, QuestObjective.Name, tostring(amountNeeded.Value))
		wait(3)
		ProgressQuest:FireClient(player, tostring(amountKilled.Value), tostring(amountNeeded.Value))
	end
end)
...
AcceptQuest.OnServerEvent:Connect(function(player, npcName)
	QuestManager.AcceptQuest(player, npcName)
end)

CompleteQuest.OnServerEvent:Connect(function(player, npcName)
	QuestManager.CompletedCheck(player, npcName)
end)
